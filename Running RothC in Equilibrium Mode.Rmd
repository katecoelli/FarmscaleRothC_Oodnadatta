...
```{r}
res = data.frame(c_input = NA, DPM=NA, RPM=NA, BIO=NA, HUM=NA, IOM=NA)    #nrow of matrix = number of cinput options * number years
results<- list()
...
lst_5_yr=tmp[(nrow(tmp)-5*12):nrow(tmp),]
lst_5_yr_avge = colMeans(lst_5_yr)
res[1] = Cinputs[i]
...
### Create a heatmap to show minimum error


```{r}

ggplot()+
  geom_tile(heat_map_errors, mapping = aes(COEFF, WUE, fill=crop_total_error))+
  scale_fill_viridis(option = "D")+
  geom_label(min_error, mapping = aes(COEFF, WUE, label = "min_error"))+
  ggsave("heatmap_crop_WUEvsCOEFF_minerror.png")
